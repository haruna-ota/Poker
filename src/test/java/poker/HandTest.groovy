...
class HandTest extends Specification {
    ...
    def sortTest() {
        expect:
        new Hand(cards).sortHand().toString() == exp

        where:
        cards                                           | exp
        [$(H, 2), $(H, 6), $(H, 4), $(H, 3), $(H, 5)]   | "[H-2, H-3, H-4, H-5, H-6]" //全てランクが同じ場合
        [$(H, 2), $(D, 2), $(S, 5), $(C, 2), $(S, 2)]   | "[S-2, H-2, C-2, D-2, S-5]" //マーク違い,ランク重複ありの場合
        [$(H, 5), $(D, 3), $(C, 10), $(H, 1), $(S, 12)] | "[H-A, D-3, H-5, C-10, S-Q]" //全てランク違い、マークも違う場合
    }
}
